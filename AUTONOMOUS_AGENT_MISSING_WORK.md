...
- Agent-facing deal-to-approval pipeline: `/api/agent/pipeline` and `scripts/run_bid_pipeline.py` run discovery through safe task advancement, return `next_agent_actions` with endpoints/payload templates/completed-action templates/resume commands, apply bounded `completed_actions`, and generate structured `generated_bid_packages` only for supplied or completed current approval request ids.
- Completed action API resume: `/api/agent/completed-actions` accepts bounded completed-action templates, applies them through the same pipeline whitelist, and returns the resumed pipeline state, next actions, applied actions, errors, and generated bid packages.
- CLI agent handoff directory: `scripts/run_bid_pipeline.py --agent-work-dir DIR` writes compact pipeline summary, next actions, completed-action templates, package manifests, and generated bid package files for terminal agents.
- Fillable report templates: `--agent-work-dir` writes `report-templates.json` and ready-to-fill report files under `reports\` for owner approvals, portal package downloads, and portal submission preparation; `--resume-agent-work-dir` ignores unfilled templates until they are completed.
- Single-directory agent resume: `scripts/run_bid_pipeline.py --resume-agent-work-dir DIR --agent-work-dir DIR` loads owner approval reports, completed action JSON, portal package reports, portal submission reports, and downloaded `OPPORTUNITY_ID__anything.pdf` packages from one handoff/work folder, de-duplicates package analysis actions, and writes the next handoff back to the same folder.
- API handoff directory export: `/api/agent/workdir-export` runs the deterministic pipeline and writes the same local `agent-work` handoff directory as the CLI, so browser/desktop agents can get compact file contracts without invoking the CLI.
- API single-directory agent resume: `/api/agent/workdir-resume` loads the same local handoff/work folder artifacts, records portal submission preparation reports, applies bounded completed actions through the deterministic pipeline whitelist, and returns the resumed next actions/generated packages for browser or desktop agents.
- Owner approval handoff reports: `--agent-work-dir` writes `owner-approval-requests.json` plus per-request files with an `owner_approval_decision_report` template; `/api/agent/owner-approval-report`, `--owner-approval-report-file`, `--owner-approval-report-dir`, and `--resume-agent-work-dir` convert explicit `approved=true` owner decisions into bounded `/api/owner-approval/approve` completed actions without accepting browser-supplied compliance or pricing rows.
...
- Portal package completion reports: each package request includes a `completion_report_template`; `/api/agent/package-report`, `scripts/run_bid_pipeline.py --portal-package-report-file REPORT.json`, and `--portal-package-report-dir DIR` verify downloaded PDFs and turn them into bounded `/api/documents/analyze` actions. `/api/agent/package-report` can also accept `resume_pipeline: true` to return the next pipeline state and next actions in the same call.
...
- Portal submission preparation reports: `/api/agent/submission-report` and `scripts\run_bid_pipeline.py --portal-submission-report-file/--portal-submission-report-dir` record copied fields, prepared attachments, and portal blockers from browser/portal agents, persist the audit, and reject any report claiming final buyer submission.
