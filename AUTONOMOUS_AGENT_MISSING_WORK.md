...
- Portal package completion reports: each package request includes a `completion_report_template`; `/api/agent/package-report`, `scripts/run_bid_pipeline.py --portal-package-report-file REPORT.json`, and `--portal-package-report-dir DIR` verify downloaded PDFs and turn them into bounded `/api/documents/analyze` actions.
