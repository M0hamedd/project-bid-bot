...
- Owner approval handoff reports: `--agent-work-dir` writes `owner-approval-requests.json` plus per-request files with an `owner_approval_decision_report` template; `--owner-approval-report-file`, `--owner-approval-report-dir`, and `--resume-agent-work-dir` convert explicit `approved=true` owner decisions into bounded `/api/owner-approval/approve` completed actions without accepting browser-supplied compliance or pricing rows.
